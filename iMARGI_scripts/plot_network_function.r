plot_network_NEW <- function(indexes, output_path, vertex_size=1, v_label_cex=1, vertex_color = "#0000ff",
                             ...
                             increase_e_size=c(), increase_e_color=c(), increase_arrow_size=c(), increase_arrow_width=c(),
                             markers_names = c(),
                             color_markers = FALSE,
                             my_layout = layout.sphere,
                             plot_chr_label = FALSE,
                             plot_vertex_label = TRUE,
                             add_single_nodes = c(),
                             resolution=300,
                             pixels=2000) 

{
  ...
  SE_network_info$SE_index = super_enhancers_sort_annotated_new$SE_index_new
  SE_network_info$SE_label = super_enhancers_sort_annotated_new$labels_new
  ...
  temp = gsub("-.*","",SE_network_info$SE_label)
  temp = gsub("C","chr",temp)
  SE_network_info$SE_chr = temp
  
  indexes_temp_row = as.character(super_enhancers_sort_annotated_new[indexes[,"row"],"SE_index_new"])
  indexes_temp_col = as.character(super_enhancers_sort_annotated_new[indexes[,"col"],"SE_index_new"])
  ...
  if (length(edges_to_increase) > 0){
    for (j in 1:length(edges_to_increase)){
      ...
      edges_to_increase_temp = cbind(as.character(super_enhancers_sort_annotated_new[edges_to_increase_j[,1],"SE_index_new"]),as.character(super_enhancers_sort_annotated_new[edges_to_increase_j[,2],"SE_index_new"]))
  ...
  if (mes_markers_names == TRUE){
    d$indexes_temp_row = mapvalues(d$indexes_temp_row,SE_mes_markers_index_full[,1],SE_mes_markers_index_full[,2],warn_missing = FALSE)
    d$indexes_temp_col = mapvalues(d$indexes_temp_col,SE_mes_markers_index_full[,1],SE_mes_markers_index_full[,2],warn_missing = FALSE)
  }
  
  graph_object = graph_from_data_frame(d, directed = FALSE, vertices = NULL)
  
  if (length(add_single_nodes)>0){
    single_nodes_label = as.character(super_enhancers_sort_annotated_new[add_single_nodes,"SE_index_new"])
  ...
  if (length(vertex_to_increase)>0){
    ...
    for (i in 1:length(vertex_to_increase)){
      vertex_to_increase_label = as.character(super_enhancers_sort_annotated_new[vertex_to_increase[[i]],"SE_index_new"])
  ...
  if (plot_chr_label == TRUE){
    ...
    for (i in hg38_chromosomes){
      se_ind = SE_network_info[which(SE_network_info$SE_chr == i),1]
  ...
  png(output_path, width = pixels, height = pixels, units = "px", res = resolution)
  #pdf(output_path, width = 8, height = 8)
  if (plot_vertex_label == TRUE){
    if (plot_chr_label == TRUE){
      p<-plot(graph_object, vertex.label = ifelse(V(graph_object)$name %in% total_vertex_to_increase, V(graph_object)$name, NA), 
              layout=my_layout,
              vertex.label.color = "black", vertex.label.cex = v_label_cex, vertex.label.family = "Helvetica",
  ...
  else if (plot_vertex_label == FALSE){
    p<-plot(graph_object, vertex.label = NA, 
            layout=my_layout,
            vertex.label.color = "black", vertex.label.family = "Helvetica", 
            edge.arrow.size = E(graph_object)$arr_size, edge.arrow.width = E(graph_object)$arr_width, edge.width = E(graph_object)$e_width, edge.color = E(graph_object)$e_color)
  }
